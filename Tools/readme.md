# Useful tools [![Awesome](https://cdn.rawgit.com/sindresorhus/awesome/d7305f38d29fed78fa85652e3a63e154dd8e8829/media/badge.svg)](https://github.com/sindresorhus/awesome) ![](https://img.shields.io/badge/igorgcustodio-tools-brightgreen)

> A list of helpful articles, videos and tutorials about management.

*Please read the [contribution guidelines](../readme.md/#guidelines) before contributing.*


## Table of Contents

- [Websites](#websites)
- [Bash](#bash)
- [APIs utilities](#apis)
- [Web Development](#web)
- [Design](#design)
- [Mac](#mac)
- [iOS](#ios)
- [Time Tracking](#time-tracking)
- [Great topics to follow](#great-topics-to-follow)

## Websites

- [4Devs - A couple of useful services for developers](https://www.4devs.com.br/)
- [Repository to help you build anything](https://github.com/danistefanovic/build-your-own-x)
- [Extension to make a couple of sites dark](https://darkreader.org)
- [RegEx utilities](http://regex101.com/)
- [Use your browser with mouse gestures](https://github.com/Robbendebiene/Gesturefy)

## Bash

- [Productivity for bash](https://github.com/ohmyzsh/ohmyzsh)
- [Depix - Reveal pixelated texts on images](https://github.com/beurtschipper/Depix)

## APIs

- [JSON Server - Local JSON-based server](https://github.com/typicode/json-server)
- [List of Public APIs](https://github.com/public-apis/public-apis)
- [Request Baskets - Service for testing HTTP Requests](https://github.com/darklynx/request-baskets)
- [Request Bin - Service for testing HTTP Requests](https://pipedream.com/requestbin)
- [Request Hub - Service for testing HTTP Requests](https://github.com/kyledayton/requesthub)

## Web

- [Pesticide - Useful extension for debug layouts - Chrome](https://chrome.google.com/webstore/detail/pesticide-for-chrome-with/neonnmencpneifkhlmhmfhfiklgjmloi)
- [Pesticide - Useful extension for debug layouts - Firefox](https://addons.mozilla.org/en-US/firefox/addon/pesticide-for-firefox/)
- [Expose a web service/site from your computer to the world](https://theboroer.github.io/localtunnel-www/)
- [Websocket Weasel](https://addons.mozilla.org/pt-BR/firefox/addon/websocket-weasel/)

## Design

- [IconDuck - free icons pack](https://iconduck.com/)
- [FlatIcon - free icons pack](https://www.flaticon.com/)
- [Unsplash - free image database](https://unsplash.com/)

## Mac

- [BlackHole - Sound channels manager - Useful for Streaming on Mac](https://github.com/ExistentialAudio/BlackHole)
- [BackgroundMusic - Sound mixer](https://github.com/kyleneideck/BackgroundMusic)
- [Aerial Screensaver - The most beautiful screensaver for Mac based on Apple TV](https://github.com/JohnCoates/Aerial)

## iOS

- [Kin - Check if your project.pbxproj is correct](https://github.com/Karumi/Kin)

## Time tracker

- [Clockify](https://clockify.me/tracker)

## Great topics to follow

- [Programming Topic on Twitter](https://twitter.com/i/topics/848921413196984320)